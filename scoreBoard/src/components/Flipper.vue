 ...
 */
<template>
  <div class="M-Flipper" :class="[flipType, {'go': isFlipping}]" :style="styleVar">
...
export default {
  ...
  methods: {
    ...
    setStyle(width, fontSize) {
          this.styleVar['--flipperWidth'] = width;
          this.styleVar['--flipperFontSize'] = fontSize;
    }
  },
  created() {
...
.M-Flipper {
  ...
  position: relative;
  width: var(--flipperWidth);
  height: 40vw;
  line-height: 40vw;
  border: solid 1px #000;
  border-radius: 10px;
  background: #fff;
  font-size: var(--flipperFontSize);
  color: #fff;
...
@keyframes frontFlipDown {
  0% {
    transform: perspective(60vw) rotateX(0deg);
  }

  100% {
    transform: perspective(60vw) rotateX(-180deg);
  }
}
